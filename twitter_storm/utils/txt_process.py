"""Module which process text to transform in a sequence of consistent tweets"""
import re


def tweet_breaker(text, max_size=140):
    """Method to initially break a string considering a tweet with 140 caracters counting with the index/total header"""
    tweets = []
...
def format_tweets(tweets_list, mask):
    """Method to transform the Tweet list in a tweet string, adding the final size in the header"""
...
def add_tweet_index(tweets_list):
    """Add the index header in each tweet"""
